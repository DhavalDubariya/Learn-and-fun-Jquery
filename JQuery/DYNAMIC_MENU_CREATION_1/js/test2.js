  ...
  $(".add_btn").on("click", function () {
    ...
    if (select_menu != "none") {
      if ($(`[data='${select_menu}_child']`).length == 0) {
        $(".right")
          .find(`[data='${select_menu}']`)
          .append(
            `<ul data="${select_menu}_length" class="${select_menu}_length" ></ul>`
          );
      ...
      $(`[data='${select_menu}_child']`).append(
        `<option class="parent_child" value="${input_menu}">${input_menu}</option>`
      );

      $(".right")
        .find(`[data='${select_menu}']`)
        .find(`[data='${select_menu}_length']`).append(`
                <li data="${input_menu}" >
                    <p>${input_menu}</p>
                    <button >Edit</button>
      ...
      $(".select_menu").append(
        `<option value="${input_menu}">${input_menu}</option>`
      );
    }
  });
});
